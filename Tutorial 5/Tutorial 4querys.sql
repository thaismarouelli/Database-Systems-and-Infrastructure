...
SELECT unitcode, COUNT(studentid) as number_of_students
FROM enrolment
WHERE STUDYYEAR > 2010
GROUP BY unitcode
HAVING COUNT(studentid) > 1;

SELECT DISTINCT TEACHERNAME, SCHEDULEHOURS, PAYRATE
FROM teacher, workload
WHERE teacher.teacherid = workload.teacherid ;
...
SELECT s.STUDENTNAME, e.UNITCODE, u.UNITNAME
FROM STUDENT S,  enrolment E, unit U
WHERE s.studentid = e.studentid and e.unitcode = u.unitcode;
...
WHERE teacher.teachername = 'IMRAN KHAN';
